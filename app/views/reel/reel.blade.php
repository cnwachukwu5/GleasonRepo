...
<fieldset class="form-horizontal">
  ...
  <form method="post" action="{{ url('/reel/results/') }}"  >
      <input type="hidden" name="metricMeasure" id="metricMeasure"/>
  <div class="tab-content" id="subpage">
    ...
    <div class="tab-pane" id="search">
      ...
      <input type="hidden" value="STD" name="unitType" id="unitType">
  </form>
  <br>
</fieldset>
...
$(document).ready(function(){
    ...
    if(selValue === "STD"){
        $('#metricMeasure').val(false);
    }else{
        $('#metricMeasure').val(true);
    }
    //alert( $('#metricMeasure').val())
    $('input:radio[name=measureUnit]').on('change', function() {
        var selValue = $('input[name=measureUnit]:checked').val();
        if(selValue === "STD"){
            $('#metricMeasure').val(false);
        }else{
